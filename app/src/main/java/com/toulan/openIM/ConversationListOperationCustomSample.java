...
public class ConversationListOperationCustomSample extends IMConversationListOperation {
    ...
    public String getConversationName(Fragment fragment,
        ...
        if (conversation.getConversationBody() instanceof YWCustomConversationBody) {
            YWCustomConversationBody body = (YWCustomConversationBody) conversation.getConversationBody();
            if (body.getIdentity().equals(SYSTEM_TRIBE_CONVERSATION)) {
                return "群系统消息";
            } else if (body.getIdentity().equals(SYSTEM_FRIEND_REQ_CONVERSATION)) {
                return "联系人系统消息";
...
//        }
        if (conversation.getConversationType() == YWConversationType.Custom) {
            YWCustomConversationBody body = (YWCustomConversationBody) conversation.getConversationBody();
            String conversationId = body.getIdentity();
            if (conversationId.startsWith(SYSTEM_FRIEND_REQ_CONVERSATION)) {
                Intent intent = new Intent(TApplication.getApplication(), ContactSystemMessageActivity.class);
                fragment.getActivity().startActivity(intent);
                return true;
            } else if (conversationId.startsWith(SYSTEM_TRIBE_CONVERSATION)) {
                Intent intent = new Intent(TApplication.getApplication(), TribeSystemMessageActivity.class);
                fragment.getActivity().startActivity(intent);
                return true;
            }
        }
